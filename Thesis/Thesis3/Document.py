...
class Document:
    ...
    #                     bridge_antecedents = [m.markable_id for m in
    #                                           self.gold_clusters[bridged_from_mention.gold_cluster_id] if m.markable_id is not None]
    #                 else:
    #                     bridge_antecedents = [bridged_from_mention.markable_id]
    #                 lines.append('\t'.join([m.underscore_span(), m.markable_id, bridged_from_mention.underscore_span(), '|'.join(bridge_antecedents)]))
    #     return '\n'.join(lines)
    #
